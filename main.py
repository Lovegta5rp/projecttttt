...
ADMIN_ID = 123456789  # Замените на ваш реальный ID (числовой)
...
async def start(update: Update, context: CallbackContext) -> None:
    ...
    await update.message.reply_text("Привет! Я бот для заказа услуг. Нажмите 'Услуги', чтобы выбрать нужную услугу, или 'Связь с администратором' для связи с администратором. 😊", reply_markup=reply_markup)
...
async def show_services(update: Update, context: CallbackContext) -> None:
    ...
    keyboard.append([InlineKeyboardButton("Связь с администратором 📞", callback_data="contact_admin")])  # Добавляем кнопку снизу
...
async def contact_admin(update: Update, context: CallbackContext) -> None:
    ...
    user_data[update.callback_query.from_user.id] = {'stage': 'contact_admin'}
    
    await update.callback_query.message.reply_text("Напишите ваше сообщение для администратора:")
